...
export default function TakePracticeQuizPage() {
  ...
  const fetchQuiz = useCallback(async () => {
    try {
      ...
      if (response.success && response.data) {
        ...
        // For Hard/GOD each question has its own timer; fall back to quiz-level for Easy/Medium
        const firstQ = response.data.questions?.[0];
        const initialTime = firstQ?.timePerQuestion ?? response.data.timePerQuestion;
  ...
  const handleNext = useCallback(() => {
    ...
    if (currentQuestionIndex < quiz.questions.length - 1) {
      ...
      // Use per-question time for Hard/GOD, otherwise use quiz-level time
      const nextQ = quiz.questions[nextIndex];
      setTimeLeft(nextQ?.timePerQuestion ?? quiz.timePerQuestion);
    } else {
      handleSubmit();
    }
  ...
  if (!hasStarted && !isFinished) {
    ...
    const isHardOrGod = quiz.difficulty === "hard" || quiz.difficulty === "god";
    return (
      <div className="min-h-screen bg-gray-50 dark:bg-gray-900 flex flex-col items-center justify-center p-4">
        <motion.div 
        ...
        >
          <div className={`w-12 h-12 sm:w-16 sm:h-16 rounded-xl flex items-center justify-center mx-auto mb-4 sm:mb-6 ${
            isGodMode ? "bg-amber-50 dark:bg-amber-900/20" : "bg-blue-50 dark:bg-blue-900/20"
          }`}>
            {isGodMode
          ...
          <p className="text-xs sm:text-sm text-gray-500 dark:text-gray-400 mb-4 sm:mb-6 leading-relaxed px-2">
            This quiz will be conducted in full screen. Do not switch tabs or windows during the test.{" "}
            <strong className="text-black dark:text-white font-medium">3 warnings will result in automatic termination.</strong>
          </p>

          {isGodMode && (
            <div className="mb-5 sm:mb-6 p-3 sm:p-4 rounded-xl bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800/40 text-left">
              <p className="text-xs sm:text-sm font-semibold text-amber-700 dark:text-amber-400 mb-1">GOD Mode Rules</p>
              <ul className="text-xs text-amber-600 dark:text-amber-300/80 space-y-1 list-disc list-inside">
                <li>Each question may have <strong>multiple correct answers</strong></li>
                <li>You must select <strong>all</strong> correct answers to earn the point</li>
                <li>No partial credit - it&apos;s all or nothing</li>
                <li>AI sets the timer for each question individually</li>
              </ul>
            </div>
          )}

          {isHardOrGod && !isGodMode && (
            <div className="mb-5 sm:mb-6 p-3 sm:p-4 rounded-xl bg-gray-50 dark:bg-gray-700/30 border border-gray-200 dark:border-gray-600 text-left">
              <p className="text-xs sm:text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1">Hard Mode</p>
              <p className="text-xs text-gray-500 dark:text-gray-400">AI has set a custom time limit per question based on its difficulty.</p>
            </div>
          )}
                    ...
                    <div className="flex items-start gap-3 sm:gap-4">
                      {isMultiSelect ? (
                        ...
                        <span className={`flex items-center justify-center w-5 h-5 sm:w-6 sm:h-6 rounded-md border-2 shrink-0 mt-0.5 transition-colors ${
                          ...
                          {isSelected && (
                            <svg className="w-3 h-3 text-black dark:text-white" fill="none" viewBox="0 0 12 12">
