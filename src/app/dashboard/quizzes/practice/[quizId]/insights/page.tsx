...
import { CheckCircle, XCircle, ChevronDown, ChevronUp, AlertTriangle, Clock, Target as TargetIcon, TrendingUp, Sparkles, Lock } from "lucide-react";
...
export default function PracticeQuizInsightsPage() {
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        if (response.attempts && response.attempts.length > 0) {
          setSelectedAttemptId(response.attempts[response.attempts.length - 1].attemptId);
        }
      } else {
  ...
  const handleUnlockAnswers = async () => {
    const confirmed = await showDialog({
      ...
      message: "You still have trials remaining. Unlocking answers will give up this quiz and you won't be able to attempt it again. Are you sure?",
  ...
  if (authLoading || loading) return <ZeroGravityLoading title="Loading Insights" showNavigation={false} />;
  if (!quiz) return null;
  ...
  const pieData = selectedAttempt ? [
    ...
    { name: "Incorrect", value: selectedAttempt.totalQuestions - selectedAttempt.score, color: "#ef4444" }
  ] : [];
  ...
  const estimatedAvgTime = selectedAttempt
    ? Math.max(5, Math.round(quiz.timePerQuestion * (0.3 + (selectedAttempt.score / selectedAttempt.totalQuestions) * 0.4)))
    : 0;
        ...
        <div className="max-w-6xl mx-auto px-4">
          ...
          {!selectedAttempt ? (
            <div className="bg-white dark:bg-gray-800 p-8 rounded-xl text-center border border-gray-100 dark:border-gray-700">
              ...
              <h2 className="text-xl font-semibold text-black dark:text-white">No Attempts Found</h2>
              <p className="text-gray-500 dark:text-gray-400 mt-2">You gave up without making any attempts.</p>
              {quiz.questions && (
                <div className="mt-8 space-y-4 text-left">
                  {quiz.questions.map((q, i) => (
                    <div key={q.questionId} className="p-4 bg-gray-50 dark:bg-gray-700/50 rounded-xl border border-gray-100 dark:border-gray-700">
                      <p className="font-medium text-black dark:text-white mb-2">{i + 1}. {q.text}</p>
                      <p className="text-sm text-green-600 dark:text-green-400 font-medium">Correct Answer: {q.options.find(o => o.key === q.correctKey)?.text}</p>
                      <p className="text-sm text-gray-600 dark:text-gray-400 mt-2 bg-white dark:bg-gray-800 p-3 rounded-lg">{q.explanation}</p>
                    </div>
                  ))}
                </div>
              )}
            ...
            <div className="space-y-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-2 sm:gap-6">
                <div className="md:col-span-2 grid grid-cols-[40%_60%] md:grid-cols-2 gap-0 md:gap-6 bg-white dark:bg-[#15151a] md:bg-transparent md:dark:bg-transparent rounded-xl border border-gray-100 dark:border-gray-800 md:border-none shadow-sm md:shadow-none p-4 sm:p-5 md:p-0">
                  ...
                  <div className="flex flex-col justify-center pr-3 sm:pr-4 border-r border-gray-100 dark:border-gray-800 md:border-none md:bg-white md:dark:bg-[#15151a] md:rounded-xl md:p-6 md:px-6 md:border md:dark:border-gray-800 md:shadow-sm md:h-full md:justify-between">
                    <div className="hidden md:block">
                      <h2 className="text-xl font-medium text-black dark:text-white mb-1">Attempt Summary</h2>
                    ...
                    <div className="flex flex-col gap-4 mt-0 md:mt-8 h-full md:h-auto justify-center">
                      <div className="flex flex-col md:flex-row md:items-center md:justify-between">
                        ...
                        <div className="flex items-baseline gap-1 mt-auto md:mt-0">
                          <span className="text-xl sm:text-3xl md:text-4xl font-bold text-black dark:text-white leading-none">{selectedAttempt.score}</span>
                          <span className="text-xs sm:text-lg font-medium text-gray-400 dark:text-gray-500">/ {selectedAttempt.totalQuestions}</span>
                        </div>
                      </div>
                      <div className="flex flex-col pt-4 border-t border-gray-100 dark:border-gray-800 md:flex-row md:items-center md:justify-between">
                        ...
                        <div className="flex items-baseline gap-1 mt-auto md:mt-0">
                          <span className="text-xl sm:text-3xl md:text-4xl font-bold text-black dark:text-white leading-none">{estimatedAvgTime}</span>
                          <span className="text-xs sm:text-lg font-medium text-gray-400 dark:text-gray-500 ml-1">secs</span>
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col items-center justify-center pl-2 sm:pl-4 md:bg-white md:dark:bg-[#15151a] md:rounded-xl md:p-6 md:px-6 md:border md:dark:border-gray-800 md:shadow-sm md:min-h-[280px]">
                    <h3 className="hidden md:block text-base font-medium text-black dark:text-white w-full text-left mb-4">Accuracy Ratio</h3>
                    <div className="w-full min-h-[140px] md:h-full md:min-h-[180px]">
                      <ResponsiveContainer width="100%" height="100%">
                        <PieChart>
                          ...
                          <RechartsTooltip
                            contentStyle={{ borderRadius: '12px', border: 'none', background: '#1c1c21', color: '#fff', boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.1)' }}
                            itemStyle={{ color: '#fff' }}
                          />
                        </PieChart>
                      </ResponsiveContainer>
                    ...
                    <div className="flex gap-2 sm:gap-4 justify-between md:justify-center w-full md:w-auto text-[10px] sm:text-xs font-medium text-gray-500 dark:text-gray-400 mt-2 px-1 md:px-0">
                      <div className="flex items-center gap-1 sm:gap-1.5"><div className="w-2 sm:w-2.5 h-2 sm:h-2.5 rounded-full bg-emerald-500"></div>Correct</div>
                      <div className="flex items-center gap-1 sm:gap-1.5"><div className="w-2 sm:w-2.5 h-2 sm:h-2.5 rounded-full bg-red-500"></div>Incorrect</div>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-white dark:bg-[#15151a] rounded-xl p-4 sm:p-6 border border-gray-100 dark:border-gray-800 shadow-sm flex flex-col items-center justify-center min-h-[240px] sm:min-h-[280px]">
                  <h3 className="text-base font-medium text-black dark:text-white w-full text-left mb-4">Attempt Progression</h3>
                    ...
                    {attempts.length > 1 ? (
                      <ResponsiveContainer width="100%" height="100%">
                        <LineChart data={lineData} margin={{ top: 5, right: 5, bottom: 5, left: -20 }}>
                          ...
                          <XAxis dataKey="name" stroke="#6b7280" fontSize={10} tickLine={false} axisLine={false} padding={{ left: 30, right: 30 }} />
                          <YAxis stroke="#6b7280" fontSize={10} tickLine={false} axisLine={false} domain={[0, totalQuestions]} allowDecimals={false} />
                          <RechartsTooltip contentStyle={{ borderRadius: '12px', border: 'none', background: '#1c1c21', color: '#fff' }} />
                          <Line type="monotone" dataKey="score" stroke="#6366f1" strokeWidth={3} dot={{ r: 4, strokeWidth: 2, fill: '#15151a' }} activeDot={{ r: 6 }} />
                        </LineChart>
                      </ResponsiveContainer>
                    ) : (
                      <div className="flex items-center justify-center h-full text-gray-400 text-xs sm:text-sm flex-col gap-2">
                        ...
                        <span className="text-center">Take another trial to see progression</span>
                      </div>
                    )}
                  </div>
                </div>
              ...
              {!answersLocked && quiz.questions && (
                <div className="space-y-4 pt-4 pb-12 sm:pb-20">
                    ...
                    return (
                      <div
                        ...
                        className={`bg-white dark:bg-[#15151a] rounded-xl border transition-colors overflow-hidden ${
                          ...
                          isUnanswered ? "border-gray-100 dark:border-gray-800" :
                              ...
                              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3 mb-4 sm:mb-6">
                                {question.options.map(opt => {
                                  ...
                                  if (isActualCorrect) {
                                    ...
                                    optionClass = "border-red-500 bg-red-50 text-red-700 dark:bg-red-900/20 dark:text-red-400";
                                  }
                                  return (
                                    <div key={opt.key} className={`p-3 sm:p-4 rounded-xl border flex items-start gap-2 sm:gap-4 ${optionClass}`}>
                                      <span className={`w-5 h-5 sm:w-6 sm:h-6 rounded-md flex items-center justify-center text-[10px] sm:text-xs font-bold shrink-0 ${
                                        isActualCorrect ? "bg-green-200 text-green-800 dark:bg-green-800 dark:text-green-200" :
                                        isSelected ? "bg-red-200 text-red-800 dark:bg-red-800 dark:text-red-200" :
                                        "bg-gray-100 text-gray-500 dark:bg-gray-800 dark:text-gray-400"
                                      }`}>
                                        {opt.key}
                                      </span>
                                      <span className="text-xs sm:text-sm md:text-base font-medium pt-0.5">{opt.text}</span>
                                    </div>
                                  );
                                })}
                              </div>
                              <div className="bg-blue-50/50 dark:bg-blue-900/10 border border-blue-100 dark:border-blue-800/30 rounded-xl p-3 sm:p-5">
                                <h4 className="text-xs sm:text-sm font-bold text-blue-800 dark:text-blue-300 uppercase tracking-wider mb-2">Explanation</h4>
                                <p className="text-blue-900 dark:text-blue-100/80 text-sm leading-relaxed">{question.explanation}</p>
                              </div>
                            </div>
                          </div>
                        </div>
                ...
                >
                  <div className="bg-white dark:bg-[#15151a] rounded-xl border border-amber-200 dark:border-amber-800/40 p-5 sm:p-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        <p className="text-sm font-semibold text-gray-900 dark:text-white mb-0.5">Answers are locked</p>
                        <p className="text-xs text-gray-500 dark:text-gray-400 leading-relaxed">
                          You still have trials remaining. Give up to reveal the correct answers and full explanations.
                        </p>
                      </div>
                    </div>
                    <button
