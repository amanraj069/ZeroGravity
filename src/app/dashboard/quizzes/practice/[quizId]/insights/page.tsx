...
import { getPracticeQuizById, PracticeQuiz, PracticeAttempt } from "@/services/practiceQuizService";
import { BackButton } from "@/components/BackButton";
import { motion } from "framer-motion";
import { CheckCircle, XCircle, ChevronDown, ChevronUp, AlertTriangle, Clock, Target as TargetIcon, TrendingUp, Sparkles } from "lucide-react";
...
export default function PracticeQuizInsightsPage() {
    ...
    try {
      ...
      if (response.success && response.data) {
        if (!response.data.isGivenUp) {
          showToast("You must give up the quiz to view insights.", "error");
  ...
  // Average time estimate (for demonstration purposes on insights)
  const estimatedAvgTime = selectedAttempt ? Math.max(5, Math.round(quiz.timePerQuestion * (0.3 + (selectedAttempt.score / selectedAttempt.totalQuestions) * 0.4))) : 0;
          ...
          {!selectedAttempt ? (
            ...
            <div className="space-y-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-2 sm:gap-6">
                {/* Mobile: Merged Stats + Pie. Desktop: Splits into 2 columns (1/3 + 1/3) */}
                      ...
                      {attempts.length > 1 ? (
                        <ResponsiveContainer width="100%" height="100%">
                          <LineChart data={lineData} margin={{ top: 5, right: 5, bottom: 5, left: -20 }}>
                            ...
                            <YAxis stroke="#6b7280" fontSize={10} tickLine={false} axisLine={false} tickFormatter={(value) => `${value}`} domain={[0, quiz.questions.length]} allowDecimals={false} />
                            <RechartsTooltip 
                              contentStyle={{ borderRadius: '12px', border: 'none', background: '#1c1c21', color: '#fff' }}
                            />
                            <Line 
                              type="monotone" 
                              dataKey="score" 
                    ...
                    >
                      ...
                      >
                        <div className="overflow-hidden">
                          <div
                            className="border-t border-gray-100 dark:border-gray-800 bg-gray-50/30 dark:bg-[#15151a]/50 px-3 sm:px-6 py-3 sm:py-6"
                          >
