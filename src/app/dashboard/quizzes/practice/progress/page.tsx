...
const getScoreColor = (score: number) => {
  ...
  if (score < 80)
    return { text: "text-lime-600 dark:text-lime-400", bg: "bg-lime-500" };
  if (score < 90)
    return {
...
export default function PracticeProgressPage() {
  ...
  const handleRefreshInsight = async () => {
    ...
    try {
      ...
      if (res.success) {
        ...
        showToast(res.message || "Failed to refresh insights", "error");
      }
    } catch (err) {
      console.error(err);
  ...
  const availableQuizzes = Array.from(
    new Map(
      filteredAttempts.map((a) => [a.topic, a.quizName || a.topic]),
    ).entries(),
  ); // [[topic, displayName], ...]

  // Filter by quiz (topic) if one is selected
  const topicFilteredAttempts =
    selectedTopic === "All"
      ? filteredAttempts
  ...
  const chartData = sortedAttempts.map((attempt, index) => ({
    ...
    topic: attempt.topic,
    quizName: attempt.quizName || attempt.topic,
  }));
      ...
      <main className="py-6 sm:py-10 px-4 max-w-6xl mx-auto w-full">
        ...
        {attempts.length === 0 ? (
          ...
          <>
            {/* AI Insights Section */}
            {selectedCategory !== "All" && (
              <div className="bg-white dark:bg-[#121216] border border-gray-200 dark:border-white/5 rounded-2xl p-6 lg:p-8 mb-3 sm:mb-8 relative overflow-hidden">
                <div className="flex items-center justify-between mb-4 sm:mb-8 relative z-10">
                  ...
                  <button
                    ...
                    className="hidden sm:flex p-2 text-gray-500 hover:text-gray-900 dark:hover:text-white rounded-full transition-colors disabled:opacity-50"
                ...
                {insightLoading ? (
                  <div className="flex flex-col items-center justify-center py-8">
                    <p className="text-gray-500 dark:text-gray-400 text-sm animate-pulse">
                      Analyzing your performance in {selectedCategory}...
                    </p>
                  </div>
                ) : insightData ? (
                  <div className="relative z-10 grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div>
                      ...
                      <p className="text-gray-600 dark:text-gray-300 text-sm leading-relaxed mb-6 sm:mb-8 hidden sm:block">
                      ...
                      <div className="flex items-center gap-2 sm:gap-0 flex-wrap sm:flex-nowrap">
                        ...
                        <button
                          onClick={handleRefreshInsight}
                          disabled={insightLoading}
                          className="flex sm:hidden p-2 text-gray-500 hover:text-gray-900 dark:hover:text-white rounded-full transition-colors disabled:opacity-50 ml-2"
                          title="Refresh Insights"
                        >
                          <RefreshCw
                            size={18}
                    ...
                    <div>
                      ...
                      <div className="space-y-4">
                        {insightData.topicScores &&
                          insightData.topicScores.map((ts, i) => {
                        ...
                        {(!insightData.topicScores ||
                          insightData.topicScores.length === 0) && (
                          <p className="text-sm text-gray-500">
                            Not enough data to estimate specific topic scores.
                          </p>
                        )}
                      </div>
                    </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-5 gap-4 mb-4">
              ...
              <div className="lg:col-span-3 bg-white dark:bg-[#121216] border border-gray-200 dark:border-white/5 rounded-2xl p-5">
                <div className="flex flex-row items-center justify-between gap-3 mb-4">
                  <h3 className="text-xs sm:text-base font-medium flex-shrink-0">
                  ...
                  {availableQuizzes.length > 0 && (
                    <select
                      ...
                      className="block sm:hidden bg-gray-100 dark:bg-[#1C1C22] text-xs font-medium text-gray-700 dark:text-gray-300 px-2 sm:px-3 py-1.5 rounded-lg border border-gray-200 dark:border-white/10 outline-none cursor-pointer focus:ring-2 focus:ring-gray-200 dark:focus:ring-white/10 hover:bg-gray-200 dark:hover:bg-[#2C2C35] transition-colors flex-shrink-0"
                    >
              ...
              <div className="lg:col-span-2 bg-white dark:bg-[#121216] border border-gray-200 dark:border-white/5 rounded-2xl p-5 flex flex-col">
                <h3 className="text-xs sm:text-base font-medium mb-2 sm:mb-6">
                  {selectedCategory} Stats
                </h3>
                <div className="flex-1 grid grid-rows-3 gap-2 sm:gap-4">
                  <div className="bg-gray-50 dark:bg-[#1C1C22] rounded-xl p-2 sm:p-4 flex items-center justify-between border border-gray-100 dark:border-white/5 transition-all hover:bg-gray-100 dark:hover:bg-[#25252D]">
                    <p className="text-xs sm:text-sm text-gray-500 dark:text-gray-400 font-medium">
                      Category Attempts
                    </p>
                    <p className="text-lg sm:text-xl font-semibold text-black dark:text-white">
                  ...
                  <div className="bg-gray-50 dark:bg-[#1C1C22] rounded-xl p-2 sm:p-4 flex items-center justify-between border border-gray-100 dark:border-white/5 transition-all hover:bg-gray-100 dark:hover:bg-[#25252D]">
                    ...
                    <p className="text-lg sm:text-xl font-semibold text-black dark:text-white">
                      {Math.round(
                        categoryStats.find(
                          (c: { category: string; averageAccuracy: number }) =>
                        ...
                        <Tooltip
                          ...
                          labelFormatter={(label, payload) => {
                            if (payload && payload.length > 0) {
                              ...
                              return `${data.date} • ${data.quizName}`;
                            }
                            return label;
                          }}
                        />
